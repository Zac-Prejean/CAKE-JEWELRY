...
def get_vfm_path(clean_sku, item_options, line_index=None):  
    ...
    elif clean_sku.startswith("NCKCLT"):  
        # For NCKCLT SKUs, no font file is needed  
        return None  
    else:  
        design_font_match = re.search(r'Design:\s*([\w\s-]+)', item_options)  
        if design_font_match:  
            ...
            if design_font in design_to_font:  
                ...
                font_filename = 'default.vfm'  # Set a default value or handle appropriately  
...
def process_row(index, row, folder_name, sku, clean_sku, qty_index, load_font, order_quantities, order_skus, timestamp, process_label, customtagID):  
    ...
    print('')  
    print(f"Processing Row: {index}, Order Number: {order_number}, SKU: {sku}")  
      
    if clean_sku.startswith("NCKCLT"):  
    ...
    print(f"Personalization Text: {personalization_text}")  
      
    lines = [line for line in personalization_text.split('\n') if line.strip()]  
    lines = [re.sub(r'Line \d+: ?', '', line).strip('\r') for line in lines]  
    lines = [line.strip() for line in lines if line.strip()]  
      
    # Extract Outside Inscription and Inside Inscription for BCT SKUs  
    outside_inscription = ""  
    inside_inscription = ""  
      
    if not lines:  
        # Create a blank image or use a placeholder image  
        image = Image.new('RGB', (50, 50), color='white')  
        save_image_with_subfolders(clean_sku, sku, order_number, index, qty_index, item_options, folder_name, image, custom_field_3, timestamp, item_qty, order_quantities, order_skus, personalization_text, outside_inscription, inside_inscription, item_name, order_date, lineID, row)  
        return  
    ...
    if clean_sku.startswith("NCK") or clean_sku.startswith("RNG"):  
        ...
        image_result = save_image_with_subfolders(clean_sku, sku, order_number, index, qty_index, item_options, folder_name, image, custom_field_3, timestamp, item_qty, order_quantities, order_skus, personalization_text, outside_inscription, inside_inscription, item_name, order_date, lineID, row)  
        if image_result is not None:  
            image_path, image_name = image_result 
  
def save_image_with_subfolders(clean_sku, sku, order_number, index, qty_index, item_options, folder_name, image, custom_field_3, timestamp, item_qty, order_quantities, order_skus, personalization_text, outside_inscription, inside_inscription, item_name, order_date, lineID, row):  
    sub_folder_name = str(custom_field_3)  
    base_folder_name = f'JEWELRY {timestamp}'  
  
    # Define image_name with qty_index to make it unique  
    image_name = f"{sku}_{lineID}_{order_number}_{qty_index}.png"  
    ...
    # Create label for the generated image  
    create_label(order_number, sku, qty_index, base_folder_name, custom_field_3, timestamp, item_qty, order_quantities, order_skus, personalization_text, outside_inscription, inside_inscription, item_name, order_date, lineID, sub_folder_path, image_path=image_path)  
    add_status(row, customtagID)  
  
    return image_path, image_name 
      
# SRN BCT and PICK&PACK
def process_grouped_rows(items, folder_name, order_number, sku_prefix, order_quantities, order_skus, timestamp, customtagID, first_lineID, row):  
    sub_folder_name = str(items[0][1]['Custom - Field 3'])  
    base_folder_name = f'JEWELRY {timestamp}'  
    base_path = os.path.join(os.path.expanduser('~\\Downloads'), base_folder_name)  
    sub_folder_path = os.path.join(base_path, sub_folder_name) if sub_folder_name else base_path  
  
    combined_qty = sum(int(row['Item - Qty']) for index, row in items)  
    line_ids = [str(row['Line - ID']).replace('.0', '') for index, row in items]  
    line_id_combined = line_ids[0]  
  
    combined_label(order_number, items, base_folder_name, timestamp, order_quantities, order_skus, sub_folder_path, line_id_combined, customtagID)  
    add_status_combined(items, combined_qty, customtagID, first_lineID)  
        ...
        try:  
            ...
            if clean_sku.startswith("NCK") or clean_sku.startswith("RNG"):  
                for qty_index in range(item_qty):  
                    customtagID = generate_custom_id_tag()  
                    process_row(index, row, full_folder_path, row['Item - SKU'], clean_sku, qty_index, load_font, order_quantities, order_skus, start_datetime, process_label=True, customtagID=customtagID)  
            else:  
                if order_number not in grouped_items:  
                    grouped_items[order_number] = {"SRN": [], "BNCK": [], "BCT": [], "OTHERS": []}  
                if clean_sku.startswith("SRN"):  
                    grouped_items[order_number]["SRN"].append((index, row))  
                elif clean_sku.startswith("BNCK"):  
                    grouped_items[order_number]["BNCK"].append((index, row))  
                elif clean_sku.startswith("BCT"):  
                    grouped_items[order_number]["BCT"].append((index, row))  
                else:  
                    grouped_items[order_number]["OTHERS"].append((index, row))  
        except Exception as e:  
            print(f"Error processing row {index}: {e}")  
            traceback.print_exc()  
  
    for order_number, sku_groups in grouped_items.items():  
        for category, items in sku_groups.items():  
            if items:  
                customtagID = generate_custom_id_tag()  
                first_item = items[0]  
                first_lineID = str(first_item[1]['Line - ID']).replace('.0', '')  
                row = first_item[1]  
                process_grouped_rows(items, full_folder_path, order_number, None, order_quantities, order_skus, start_datetime, customtagID, first_lineID, row)  
